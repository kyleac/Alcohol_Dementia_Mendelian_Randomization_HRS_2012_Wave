...
```{r }
#raw.summary <- read_tsv(here("data", "chambers_ggt_GCST001277.txt"))
#head(raw.summary)

#write.csv(raw.summary, file = here("data", "chambers_raw.csv"), row.names = F)

raw.summary <- read.csv(here("data", "chambers_ggt_GCST001277.csv"))
head(raw.summary)
```
...
```{r}
str_extract(raw.summary$X95..CI..TEXT., "\\d.\\d\\d") %>% as.numeric
```
...
summary <-
  ...
  rename(
    ...
    effect_allele = "alternate.allele",
    other_allele = "reference.allele",
    eaf = "risk.allele.frequency",
    pval = "p.value",
    gene = "mapped_gene") %>%
  mutate(
    se = (((str_extract(raw.summary$X95..CI..TEXT., "\\d.\\d\\d") %>% as.numeric()) + beta)/1.96),
    beta = beta/100,
    se = se/100,
    samplesize = 61089
  ) %>%
  mutate(pve = (2*beta^2*eaf*(1-eaf))/(2*beta^2*eaf*(1-eaf)+se^2*2*samplesize*eaf*(1-eaf))) %>%
  mutate(fstat_snp = ((samplesize-1-1)*pve)/1/(1-pve)^2) %>%
  mutate(fstat = ((samplesize-nrow(summary)-1)*(sum(pve))/nrow(summary)/(1-sum(pve))^2))
...
```{r}
dat.out <- format_data(type = "outcome", summary)
# By default, clumps all index and secondary SNPs at a 10000kb resolution with r^2 > .001
#dat.exp.clump <- clump_data(dat.exp)
```
...
dat <- harmonise_data(alc_exp_dat, dat.out)
```

...
Coding attempts to create an ever drank variable
```{r, eval = F, include = F}
dat <-
  dat %>%
  rowwise() %>%
  mutate(everdranksum =
          sum(R3DRINK, R4DRINK, R5DRINK, R6DRINK, R7DRINK, R8DRINK, R9DRINK, R10DRINK, R11DRINK, na.rm = TRUE)) %>%
  mutate(everdrank = )
```
...
test <- dat.merged %>%
  ...
  mutate(everdranksum =
          sum(R3DRINK, R4DRINK, R5DRINK, R6DRINK, R7DRINK, R8DRINK, R9DRINK, R10DRINK, R11DRINK, na.rm = TRUE)) %>%
  dplyr::select(everdranksum)
summary(test$everdranksum)
```
...
test <- dat.merged %>%
  rowwise() %>%
  mutate(allna =
          is.na(R3DRINK, R4DRINK, R5DRINK, R6DRINK, R7DRINK, R8DRINK, R9DRINK, R10DRINK, R11DRINK))
  dplyr::select(everdranksum)
summary(test$everdranksum)

is.na(dat$R3DRINK) %>% summary
```


Code chunk to create categorized exposure category when ready
```{r, eval = F, include = F}
...
#g.cind.tidy <- tidy(fitIV_g, conf.int = TRUE, exponentiate = TRUE)
#g.cind.tidy$model <- "g-estimation, CIND"
```

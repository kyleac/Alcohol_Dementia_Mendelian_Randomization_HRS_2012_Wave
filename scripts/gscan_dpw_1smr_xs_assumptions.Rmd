...
```{r, eval = F, include = F}
ggplot(data = fit_tidy_df, mapping = aes(x = estimate, y = -log10(Wald.p.value), color = predictor)) + geom_point() + ylim(c(0, 12.5)) + geom_hline(yintercept = 1.3, linetype = 'dotted') + ggtitle("Simple linear regression between instrument dosage and potential linear confounders", subtitle = paste0("European ancestries, n = ", nrow(dat.ea))) + labs(caption = "Additional ancestries hits truncated") + xlab("Beta coefficient") + ylab("-log10 p-value")
```
